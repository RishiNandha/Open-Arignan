...
from arignan.models import LoadEvent, LoadOperation, ParsedDocument

from .discovery import discover_sources
from .log import IngestionLog
from .parsers import DocumentParser, UrlFetcher
...
class IngestionService:
    ...
    def ingest(
        ...
        documents = [self.parser.parse(source, load_id=resolved_load_id, hat=hat) for source in sources]
        batch = IngestionBatch(
            load_id=resolved_load_id,
            hat=hat,
            input_ref=str(input_ref),
            source_items=[source.source_uri for source in sources],
            documents=documents,
        )
        if log_event:
